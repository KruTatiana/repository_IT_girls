const ratingEl = document.querySelector('.ratingEl');
...
class ComicsCard {
  constructor (name, universe, alterego, occupation, friends, superpowers, url,
    ...
    getCard() {
      ...
      this.universeEl = document.createElement('p');
      this.alteregoEl = document.createElement('p');
      this.occupationEl = document.createElement('p');
      this.friendsEl = document.createElement('p');
      this.superpowersEl = document.createElement('p');
      ...
      this.element.appendChild(this.universeEl);
      this.element.appendChild(this.alteregoEl);
      this.element.appendChild(this.occupationEl);
      this.element.appendChild(this.friendsEl);
      this.element.appendChild(this.superpowersEl);
      this.element.appendChild(this.pictureEl);
      this.element.appendChild(this.infoEl);
      this.element.setAttribute('class','super-card');
      this.headerEl.innerText = this.name;
      this.headerEl.setAttribute('class','name');
      this.universeEl.innerText = this.universe;
      this.alteregoEl.innerText = this.alterego;
      this.occupationEl.innerText = this.occupation;
      this.friendsEl.innerText = this.friends;
      this.superpowersEl.innerText = this.superpowers;
      this.pictureEl.setAttribute('src',this.url);
      this.infoEl.innerText = this.info;
    }

    getStars() {
      let personalRating = ratingEl.cloneNode(true);
      this.element.appendChild(personalRating);
      let memoryRating = localStorage.getItem('ratings');
      memoryRating = JSON.parse(memoryRating);
      memoryRating.forEach(item =>{
        if(item.name == this.name) {
          let ratingContainer = this.element.lastElementChild;
          let ratingBody = ratingContainer.children[0];
          let ratingLine = ratingBody.firstElementChild;
          ratingLine.style.width = `${item.ratingWidth}%`;
        }
      });
    }
}
...
function makeCards() {
  dateArray.forEach(date => {
    let cardObject = new ComicsCard (date.name, date.universe, date.alterego, date.occupation, date.friends, date.superpowers, date.url, date.info);
    cardObject.getCard();
    cardObject.getStars();
    });
  }
...
function getStars(el) {
  ...
  let ratingActiveWidth = itemValue * 20;
  let itemsContainer = el.parentElement;
  let ratingLine = itemsContainer.previousElementSibling;
  console.log(ratingLine);
  ratingLine.style.width = `${ratingActiveWidth}%`;
  let ratingBody = ratingLine.parentElement;
  let ratingContainer = ratingBody.parentElement;
  let card = ratingContainer.parentElement;
  let heroName = card.firstElementChild.innerHTML;
  let heroObj = {};
  heroObj.name = heroName;
  heroObj.ratingWidth = ratingActiveWidth;
  newArr.push(heroObj);
  window.localStorage.setItem('ratings',JSON.stringify(newArr));
}
